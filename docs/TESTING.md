...
|---|---|---|---|---|
| Backend hermetic | `backend/tests/` | 1405 | none (all I/O mocked) | Backend (pytest + recall gate) |
| Recall gate | `backend/tests/analysis/` | 597 (subset of the above) | none | Backend (run first, on its own) |
| Live-DB tier | `backend/tests/live_db/` | 47 | ClickHouse + Postgres | Live-DB tier (ClickHouse 26.x + Postgres) |
...
That is 2,139 tests across the seven independent tiers (the recall gate is a
