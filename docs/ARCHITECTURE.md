...
|---|---|---|---|
| ClickHouse | **Analytics Warehouse** | ReplacingMergeTree, deliberately UNpartitioned: dedup only happens within a partition and no time column is stable across replays, so a time partition would scatter versions of one logical row where `FINAL` could never collapse them (`assert_no_legacy_schema` refuses a partitioned table at startup) | Structured blockchain facts: `transactions`, `transaction_inputs`, `transaction_outputs`, `address_transactions` (MV), `tx_class_scores` (9-class score vectors plus the `contract_address` the winning class implicates, which the grouped alerts view groups on), `baselines` (per-script/per-policy/global percentile baselines). Append-only. `total_input_value` is `Nullable(UInt64)`; `NULL` means unresolved (tx confirmed without prior mempool observation); non-NULL for mempool-observed txs where inputs were resolved via `queryLedgerState/utxo`. |
...
- **Topology:** it shares the same ClickHouse server as the host but owns a separate database (`tms_clustering`). By default (`CHAIN_SOURCE=host_ch`) it **reads** chain facts from the host's Analytics Warehouse (`tms_analytics`, no chain data is duplicated) and **writes** its own state (`cluster_models`, `tx_classifications`, and the `tx_contract_anomaly` projection). Set `CHAIN_SOURCE=blockfrost` and it instead downloads an address's transaction history over HTTP (blockfrost.io) into `tms_clustering` on demand, for clustering contracts the host has not ingested (the automatic feed is host_ch-only).
- **How the host consumes it (read-time overlay, recall-first):** the host derives every score at read time and never trusts a precomputed one. `db/clustering_queries.py` reads the raw verdicts from `tx_contract_anomaly`; `analysis/contract_anomaly.py` projects them onto the 0-100 host scale via `config/detection.yaml` (so a calibration change there applies to all historical rows with no backfill, and `benign` / `normal` verdicts suppress to 0 while only `malicious` / `anomaly` carry a score); `api/analysis.py` merges that into results additively (it can only ever **raise** a tx's score/band, never lower it), supports filtering the list by `attack_class=contract_anomaly`, and reconciles the list view, the contract-grouped alerts view, band-count KPIs, and the alert timeseries. Grouping is where the overlay is most visible: the SQL grouping keys on the STORED contract while a winning verdict installs its own watched target, so a transaction can be counted under a group SQL never produced. The Validators / cluster-graph UI talks to the sidecar through the `api/clustering.py` reverse-proxy (same-origin, session-authed; the sidecar is never exposed publicly).
