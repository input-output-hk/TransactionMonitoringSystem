...
`resolve_baseline_or_fixed()` tries dynamic baselines (per-script -> global) first. Fixed anchors are used only when no dynamic baseline exists. This allows learned baselines to override the spec defaults as the system accumulates data.
