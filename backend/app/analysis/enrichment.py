...
def enrich_cycle_features(rows: list[dict[str, Any]], network: str):
    ...
    for row in rows:
        ...
        try:
            ...
            if cycle:
                row["cycle"] = cycle
                # Found, but scored without its origin's history: retry for it,
                # and if the read keeps failing the engine writes this score with
                # a marker rather than dropping the cycle.
