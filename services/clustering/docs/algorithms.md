...
with no extra API calls) and falls back to the raw address for enterprise / pointer /
...
| What counts as a vote | iso + lof + **DBSCAN-noise** (3, computed over the live population, so genuinely independent) | iso + lof **only** (0–2). The "online-noise" flag (`cluster_id = -1`) is **not** a vote: it is collinear with iso/lof novelty, so counting it would triple-count one "far from training" fact |
...
| **Graph scale** | The Jaccard matrix is dense O(n²); above `MAX_GRAPH_TXS` (5000) the tx set keeps the **most recent transactions by `block_time`** (older history is dropped and the drop is surfaced in the run notes). Use `combined` for full coverage on large targets. | Medium |
