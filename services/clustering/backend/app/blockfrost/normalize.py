...
def _collect_io(
    ...
    assets = [
        AssetRecord(target, tx_hash, role, idx, unit, qty)
        for unit, qty in _non_lovelace(amount)
    ]
    return utxo, assets
...
def build_records(
    ...
    inputs = sorted(
        (
            io
            for io in utxos.get("inputs", [])
            if _is_real_input(io, valid_contract=valid_contract)
        ),
        key=_input_sort_key,
    )
    ...
    tx = TxRecord(
        ...
        distinct_input_addresses=len(
            {r.address for r in utxo_rows if r.role == "input"}
        ),
        distinct_output_addresses=len(
            {r.address for r in utxo_rows if r.role == "output"}
        ),
