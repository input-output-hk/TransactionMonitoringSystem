...
# --- Blockfrost adapter (real BlockfrostSource over an httpx.MockTransport) ----

# Serves the canonical scenario above in Blockfrost's JSON wire format, so the
# real BlockfrostSource + AsyncBlockfrostClient run against it with no network.
...
def _make_blockfrost() -> tuple[ChainSource, Scenario]:
    ...
    return (
        BlockfrostSource(settings, client=AsyncBlockfrostClient(settings, client=inner)),
        _SCENARIO,
    )
