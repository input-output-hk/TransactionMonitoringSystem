...
async def test_source_metadata_translates_not_found() -> None:
    ...
    source = BlockfrostSource(Settings(BLOCKFROST_PROJECT_ID="t"), client=StubClient(missing=("address",)))
...
async def test_discovery_not_found_translated_to_source_error() -> None:
    ...
    with pytest.raises(SourceNotFound):
        async for _ in source.tx_hash_pages(
            address="addr1x", policy_id=None, cursor=None, mode="restart", max_items=None,
            from_block=None, to_block=None, progress=lambda _m: None,
        ):
            pass
...
@pytest.mark.parametrize(
    "cursor",
    [None, "", "point:12.ab", "page:", "page:abc", "page:3;from:", "page:3;from:abc",
     "page:3;from:-5", "page:3;from:0", "page:3;from:³"],
)
