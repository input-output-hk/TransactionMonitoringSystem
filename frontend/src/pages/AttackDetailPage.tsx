    ...
    return (
      <div className="rounded-xl border border-border bg-card p-8 text-center">
        ...
        <Link
          ...
          className="mt-4 inline-flex h-10 items-center justify-center rounded-md border border-border px-4 text-sm font-medium text-foreground hover:bg-accent"
        >
          Back
        </Link>
      </div>
          ...
          <TwoCol
            left={
              <Stack title="Collision Details">
                ...
                <KeyVal
                  ...
                  value={
                    <span className="text-status-online">0.34 ADA</span>
                  }
                />
                <div className="flex justify-start pl-1 py-1">
                  <ArrowDown className="h-4 w-4 text-brand" />
                </div>
                <KeyVal
                  label={
                    <span className="text-status-offline">LOOSER</span>
                  }
                  value={
                ...
                <KeyVal
                  ...
                  value={
                    <span className="text-status-offline">0.22 ADA</span>
                  }
                />
              </Stack>
            }
            right={
              <Stack title="Race Metrics">
                ...
                <KeyVal
                  label="OUTCOME"
                  value="TX_B_CONFIRMED (Attacker Won)"
                />
      ...
      <div className="space-y-3">
        {left.map((m, i) => (
          <div key={i}>
            ...
            {i < left.length - 1 && <div className="mt-3"><Divider /></div>}
          </div>
        ))}
      </div>
      <div className="space-y-3">
        {right.map((m, i) => (
          <div key={i}>
            <KeyVal label={m.label} value={m.value} />
            {i < right.length - 1 && <div className="mt-3"><Divider /></div>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      {label && (
        <div
          className={cn(
            'mb-1 flex items-center gap-2 text-xs font-medium uppercase tracking-wide',
            variant === 'danger'
              ? 'text-status-offline'
              : variant === 'warn'
                ? 'text-status-warning'
...
function ArrowsRow({ direction }: { direction: 'down' | 'up' }) {
  ...
  return (
    <div className={cn(FLOW_GRID, 'py-2 text-brand')}>
...
function FlowChain({
  ...
  return (
    <div className="space-y-1">
      {rows.map((r, i) => (
        <div key={i}>
          <div className={cn(FLOW_GRID, 'text-sm text-brand')}>
            <span className="min-w-0 justify-self-center font-semibold">
              {r.label}:
            </span>
            <span className="min-w-0 justify-self-center">{r.amount}</span>
            <span className="min-w-0 max-w-full justify-self-center truncate font-mono text-xs">
              {r.address}
            </span>
          </div>
      ...
      <DialogContent showClose={false} className="max-w-sm">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            className="text-brand bg-transparent border border-border hover:bg-accent hover:text-brand"
          >
            Confirm
          </Button>
        </DialogFooter>
